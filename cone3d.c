...
int AlocarCone(double r, double h, int nFaces, ConeMesh * cone)
{
    int  count = 0;
    int count2 = 0;

    int i;
    double theta = 2.01*M_PI/(double)nFaces;
    Ponto3D p;

    cone->nVert =(nFaces+1)*2 + 2;

    // nFaces * 2 (o cilindro tem 2 circulos)
    cone->ntFaces=nFaces*2;

    cone->TriArr = (tri *)malloc(sizeof(tri) * cone->ntFaces);
    cone->VertArr = (Ponto3D *)malloc(sizeof(Ponto3D) * cone->nVert);

    if((cone->VertArr == NULL)||(cone->TriArr == NULL))
        return(0);

    p.x = 0;
    p.y = 0;
    p.z = 0;
    cone->VertArr[count++] = p;
    p.x = 0;
    p.y = 0;
    p.z = h;
    cone->VertArr[count++] = p;
    ...
    for (i = 0; i <= nFaces; i++)
    {
        ...
        if(i < nFaces)
        {
            ...
            cone->TriArr[count2][0] = 0;
            cone->TriArr[count2][1] = i*2+4;
            cone->TriArr[count2][2] = i*2+2;
            count2 += 1;

            cone->TriArr[count2][0] = 1;
            cone->TriArr[count2][1] = i*2+3;
            cone->TriArr[count2][2] = i*2+5;
            count2 += 1;
...
void DrawConeMesh(ConeMesh cone)
{
    Ponto3D points[4];
    Ponto3D N;
    double l;
    int i;
    ProjetaVert(tr,cone.VertArr,cone.nVert,1.2);

    for(i=0; i < cone.ntFaces; i++)
    {
        points[0] =tr[cone.TriArr[i][0]];
        points[1] =tr[cone.TriArr[i][1]];
        points[2] =tr[cone.TriArr[i][2]];
...
int main()
{
    ...
    int nTriangulos = 16;
    int canto;
    char msg[]="Use as teclas de seta para girar o cone (arrow keys to rotate the cone - q to quit)";
    double angulo = 15;
    if(AlocarCone(Raio/1.4,Raio,nTriangulos,&cone))
    {

        tr = (Ponto3D *)malloc(sizeof(Ponto3D) * cone.nVert);
        if(tr == NULL)
        {
